...
- **Solidity stack** (pool, factory, router, oracle, lp-token, mocks)
  ...
  24,265 bytes / 24,576 bytes (311 bytes of headroom; `optimizer.runs = 999`,
  `viaIR = true`, `metadata.appendCBOR = true`, `evmVersion = "cancun"`,
  solc 0.8.36; snapshot 2026-09-14 after mint rounding and signed-cast cleanup).
  Router runtime is 22,673 bytes.
...
Post-genesis mints price LP shares on token0 and round the matching token1
payment up. If token1's requested maximum binds, token0 rounds down instead;
neither maximum is exceeded. Share issuance and donation-buffer scaling
remain rounded down. Router zap-in previews and Rust use the identical rule.
